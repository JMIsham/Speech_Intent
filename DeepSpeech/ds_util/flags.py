...
def create_flags():
    ...
    tf.app.flags.DEFINE_string  ('checkpoint_dir',   'models/checkpoint',          'directory in which checkpoints are stored - defaults to directory "deepspeech/checkpoints" within user\'s data home specified by the XDG Base Directory Specification')
    ...
    tf.app.flags.DEFINE_string  ('alphabet_config_path', 'models/ds_alphabet.txt', 'path to the configuration file specifying the alphabet used by the network. See the comment in data/alphabet.txt for a description of the format.')
    tf.app.flags.DEFINE_string  ('lm_binary_path',       'models/lm.binary', 'path to the language model binary file created with KenLM')
    tf.app.flags.DEFINE_string  ('lm_trie_path',         'models/trie', 'path to the language model trie file created with native_client/generate_trie')
