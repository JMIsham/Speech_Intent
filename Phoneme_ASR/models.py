...
class PretrainedModel(torch.nn.Module):
	...
	def compute_phoneme_logits(self, x):
		if self.is_cuda:
			x = x.cuda()
		...
		phoneme_logits = self.phoneme_linear(out)
		phoneme_logits = phoneme_logits.view(phoneme_logits.shape[0] * phoneme_logits.shape[1], -1)
		phoneme_logits = self.softmax_layer(phoneme_logits)
...
class Model(torch.nn.Module):
	...
	def decode_phonemes(self, x):
		phoneme_logits = self.pretrained_model.compute_phoneme_logits(x)
		return phoneme_logits
